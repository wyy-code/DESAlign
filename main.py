...
from src.utils import set_optim, Loss_log, pairwise_distances, csls_sim, get_adjr
...
class Runner:
    ...
    def model_choise(self, load_name=None, plug=False):
        assert self.args.model_name in ["EVA", "MCLEA", "MSNEA", "DESAlign"]
        if self.args.model_name == "DESAlign":
    ...
    def data_init(self):
        ...
        file_path = osp.join(self.args.data_path, self.args.data_choice, self.args.data_split)
        ...
        # # ##### Constructing Sparse Relation Propagation
        # dr = {}
        # for x,r,y in self.all_triples:
        #     if r not in dr:
        #         dr[r] = 0
        #     dr[r] += 1
            
        # self.sparse_rel_matrix = []
        # for i in range(self.node_size):
        #     self.sparse_rel_matrix.append([i,i,np.log(len(self.all_triples)/self.node_size)]);
        # for h,r,t in self.all_triples:
        #     self.sparse_rel_matrix.append([h,t,np.log(len(self.all_triples)/dr[r])])
        
        # self.sparse_rel_matrix = np.array(sorted(self.sparse_rel_matrix,key=lambda x:x[0]))
        # self.sparse_rel_matrix = torch.sparse_coo_tensor(indices=self.sparse_rel_matrix[:,:2].T,
        #                                             values=self.sparse_rel_matrix[:,2],
        #                                             size=(self.node_size,self.node_size)).type(torch.FloatTensor).to(self.args.device)
    ...
    def il_for_ea(self):
        with torch.no_grad():
            if self.args.model_name in ["DESAlign"]:
    ...
    def test(self, save_name="", last_epoch=True):
        ...
        self.logger.info(" --------------------- Sinkhorn Test result depth = 1 --------------------- ")
        self._test_sinkhorn(test_left, test_right, depth = 1, last_epoch=last_epoch, save_name=save_name)
        self.logger.info(" --------------------- Sinkhorn Test result depth = 2 --------------------- ")
        self._test_sinkhorn(test_left, test_right, depth = 2, last_epoch=last_epoch, save_name=save_name)
        self.logger.info(" --------------------- Sinkhorn Test result depth = 3 --------------------- ")
        self._test_sinkhorn(test_left, test_right, depth = 3, last_epoch=last_epoch, save_name=save_name)
        self.logger.info(" --------------------- Sinkhorn Test result depth = 4 --------------------- ")
        self._test_sinkhorn(test_left, test_right, depth = 4, last_epoch=last_epoch, save_name=save_name)
        self.logger.info(" --------------------- Sinkhorn Test result depth = 5 --------------------- ")
        self._test_sinkhorn(test_left, test_right, depth = 5, last_epoch=last_epoch, save_name=save_name)
    ...
    def _test(self, test_left, test_right, last_epoch=False, save_name="", loss=None, test=False):
        ...
        if self.args.csls is True:
            # # ######## Relation Modal Propagation
            # # ####FB Dataset: 2; DB Dataset: 1
            # depth = 1
            # sims = csls_sim(1 - distance, self.args.csls_k)
            # for i in range(depth):    
            #     final_emb = torch.sparse.mm(self.adj,final_emb)
            #     final_emb = F.normalize(final_emb)
            #     sims += csls_sim(1 - pairwise_distances(final_emb[test_left.cpu().data.numpy()], final_emb[test_right.cpu().data.numpy()]), self.args.csls_k)
            # sims /= depth+1
            # distance = 1 - sims
    ...
    def _test_sinkhorn(self, test_left, test_right, depth = 1, last_epoch=False, save_name="", loss=None, test=False):
        with torch.no_grad():
            w_normalized = None
            if self.args.model_name in ["EVA", "MCLEA"]:
                if self.args.model_name == "EVA":
        ...
        for epoch in range(len(sims) // batch_size + 1):
            ...
            results.append(torch.stack(torch.where(torch.eq(rank.cpu().data, torch.tensor(ans_rank, dtype=torch.long)
                                                            .unsqueeze(1)))).T.numpy())
        ...
        hits1 = hits1/len(sims)*100
        hits10 = hits10/len(sims)*100
        mrr = mrr/len(sims)*100

        self.logger.info(f"Sinkhorn Results | hits@1 = {hits1:.3f}, hits@10 = {hits10:.3f}, mrr = {mrr:.3f}")
